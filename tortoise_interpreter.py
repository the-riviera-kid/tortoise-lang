...
def navigate_command_functions(COMMANDS, program):
    ...
    for i, command in enumerate(program, 1):
        new_pen_state = get_pen_state(command, pen_state)
        command_to_parse.append(f'{i}. {COMMANDS[command[0]](command, pen_state, new_pen_state)}')
        pen_state = new_pen_state
    return command_to_parse


def pen_up(command, pen_state, new_pen_state):
    if new_pen_state != pen_state:
        return f'PEN UP'
    return f'PEN UP (ignored)'


def pen_down(command, pen_state, new_pen_state):
    if new_pen_state != pen_state:
        return f'PEN DOWN'
    return f'PEN DOWN (ignored)'


def get_pen_state(command, pen_state):
        if command[0] == 'D':
            return 'DOWN'
        elif command[0] == 'U':
            return 'UP'
...
def pen_colour(command, pen_state, new_pen_state):
    return f'P {command[1]}'


def move_north(command, pen_state, new_pen_state):
    return format_direction_and_units(command, pen_state, "north")


def move_south(command, pen_state, new_pen_state):
    return format_direction_and_units(command, pen_state, "south")


def move_east(command, pen_state, new_pen_state):
    return format_direction_and_units(command, pen_state, "east")


def move_west(command, pen_state,new_pen_state):
